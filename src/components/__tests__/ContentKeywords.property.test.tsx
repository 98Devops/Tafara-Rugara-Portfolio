...
describe('Content Keyword Validation Property Tests', () => {
      ...
      fc.assert(
        fc.property(
          ...
          (data: PortfolioData) => {
            ...
            const summaryMatches = countMatches(
              combinedSummaryText,
              requiredKeywords
            );
            expect(summaryMatches).toBeGreaterThanOrEqual(
              Math.ceil(requiredKeywords.length / 2)
            );
  ...
  describe('Experience Highlights Keywords', () => {
    it('Property 3.10: DevOps Engineer experience contains all required technical highlights', () => {
      fc.assert(
        fc.property(fc.constant(portfolioData), (data: PortfolioData) => {
          const devopsExperience = data.experience.find(exp =>
            exp.position.includes('DevOps')
          );
        ...
        fc.property(fc.constant(portfolioData), (data: PortfolioData) => {
          ...
          coreKeywords.forEach(keyword => {
            ...
            data.capabilities.forEach(cap => {
              const capText =
                `${cap.description} ${cap.skills.join(' ')}`.toLowerCase();
