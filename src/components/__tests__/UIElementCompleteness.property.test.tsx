      ...
      fc.assert(
        fc.property(
          ...
          currentPath => {
            ...
            const desktopNavLinks = container.querySelectorAll(
              '[class*="hidden md:flex"] a'
            );
      ...
      fc.assert(
        fc.property(
          ...
          ({ personalData, currentPath }) => {
            ...
            const { container: heroContainer, unmount: unmountHero } = render(
              <Hero personal={personalData} />
            );

            // Test Navigation component links separately
            const { container: navContainer, unmount: unmountNav } = render(
              <Navigation />
            );
      ...
      fc.assert(
        fc.property(
          ...
          ({ personalData, currentPath }) => {
            ...
            const viewProjectsBtn = heroContainer.querySelector(
              'a[href="/projects"]'
            );
            expect(viewProjectsBtn).toBeInTheDocument();
            expect(viewProjectsBtn?.textContent?.trim()).toBe('View Projects');
            ...
            // 5. LinkedIn social link
            const linkedinLink = heroContainer.querySelector(
              `a[href="${personalData.socialLinks.linkedin}"]`
            );
