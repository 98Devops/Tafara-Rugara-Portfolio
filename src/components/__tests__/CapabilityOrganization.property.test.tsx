...
describe('Capability Organization Property Tests', () => {
   ...
   * technical skills into exactly five structured pillars with all required
   * skills present in each category.
   */

  describe('Five Structured Pillars Organization', () => {
    it('Property 6.1: Content_Manager organizes capabilities into exactly five structured pillars', () => {
      fc.assert(
        fc.property(fc.constant(portfolioData), (data: PortfolioData) => {
          // Requirements 3.1: Five structured pillars
          expect(data.capabilities).toHaveLength(5);

          // Verify the exact five required categories are present
    ...
    it('Property 6.3: Cloud Architecture pillar contains all required skills from Requirements 3.2', () => {
      fc.assert(
        fc.property(fc.constant(portfolioData), (data: PortfolioData) => {
          ...
          const allSkillsText = cloudArchCapability!.skills
            .join(' ')
            .toLowerCase();
          const descriptionText =
            cloudArchCapability!.description.toLowerCase();
...

          {
            ...
            expect(matchCount).toBeGreaterThanOrEqual(
              Math.ceil(requiredSkillAreas.length / 2)
            );
          }
    ...
    it('Property 6.4: DevOps & CI/CD pillar contains all required skills from Requirements 3.3', () => {
      fc.assert(
        fc.property(fc.constant(portfolioData), (data: PortfolioData) => {
          ...
          const allSkillsText = devopsCapability!.skills
            .join(' ')
            .toLowerCase();
    ...
    it('Property 6.5: Automation Engineering pillar contains all required skills from Requirements 3.4', () => {
      fc.assert(
        fc.property(fc.constant(portfolioData), (data: PortfolioData) => {
          ...
          const allSkillsText = automationCapability!.skills
            .join(' ')
            .toLowerCase();
          const descriptionText =
            automationCapability!.description.toLowerCase();
          const combinedText = `${allSkillsText} ${descriptionText}`;
    ...
    it('Property 6.6: Monitoring & Reliability pillar contains all required skills from Requirements 3.5', () => {
      fc.assert(
        fc.property(fc.constant(portfolioData), (data: PortfolioData) => {
          ...
          const allSkillsText = monitoringCapability!.skills
            .join(' ')
            .toLowerCase();
          const descriptionText =
            monitoringCapability!.description.toLowerCase();
          const combinedText = `${allSkillsText} ${descriptionText}`;
      ...
      fc.assert(
        fc.property(fc.constant(portfolioData), (data: PortfolioData) => {
          data.capabilities.forEach(capability => {
            ...
            // Category should be one of the five expected values
        ...
        fc.property(fc.constant(portfolioData), (data: PortfolioData) => {
          ...
          data.capabilities.forEach(capability => {
            ...
            const expectedKeywords =
              categoryKeywords[
                capability.category as keyof typeof categoryKeywords
              ];
    ...
    it('Property 6.9: Capability organization structure supports Content_Manager display requirements', () => {
      fc.assert(
        fc.property(fc.constant(portfolioData), (data: PortfolioData) => {
          ...
          // Should be exactly 5 capabilities for grid layout
          expect(data.capabilities).toHaveLength(5);
          ...
          data.capabilities.forEach(capability => {
            ...
            expect(capability.skills.length).toBeGreaterThan(3);
            expect(capability.skills.length).toBeLessThan(15); // Reasonable for UI
          ...
          expect(uniqueCategories.size).toBe(5); // All categories are unique
        ...
        fc.property(fc.constant(portfolioData), (data: PortfolioData) => {
          data.capabilities.forEach(capability => {
            ...
            if (categoryLower.includes('cloud')) {
              ...
              const hasCloudTerms = cloudTerms.some(term =>
            ...
            if (categoryLower.includes('devops')) {
              ...
              const hasDevOpsTerms = devopsTerms.some(term =>
                combinedText.includes(term)
              );
            ...
            if (categoryLower.includes('automation')) {
              ...
              const hasAutomationTerms = automationTerms.some(term =>
                combinedText.includes(term)
              );
            ...
            if (categoryLower.includes('monitoring')) {
              ...
              const hasMonitoringTerms = monitoringTerms.some(term =>
                combinedText.includes(term)
              );
              expect(hasMonitoringTerms).toBe(true);
            }

            if (categoryLower.includes('website')) {
              // Website Development should focus on web technologies
              const websiteTerms = [
                'website',
                'web',
                'next.js',
              ...
              const hasWebsiteTerms = websiteTerms.some(term =>
                combinedText.includes(term)
              );
              expect(hasWebsiteTerms).toBe(true);
            }
          });
        }),
        { numRuns: 10 }
