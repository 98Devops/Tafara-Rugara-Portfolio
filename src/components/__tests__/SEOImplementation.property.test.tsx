...
import { generatePageMetadata, generateStructuredData, pageSEOConfig, PageSEOConfig } from '@/components/SEO';
...
describe('SEO Implementation Property Tests', () => {
  ...
  describe('Meta Tags and Open Graph Implementation', () => {
    it('Property 9.1: all pages have complete meta tags with required SEO elements', () => {
      fc.assert(
        fc.property(
          fc.constantFrom('home', 'whatIDo', 'projects', 'experience', 'contact'),
      ...
      fc.assert(
        fc.property(
          ...
          (pageKey: keyof PageSEOConfig) => {
            ...
            expect(metadata.alternates!.canonical).toContain('https://tafara-rugara.com');
            
            // Validate metadata base
            expect(metadata.metadataBase).toBeDefined();
            expect(metadata.metadataBase!.toString()).toBe('https://tafara-rugara.com/');
          }
        ),
        { numRuns: 10 }
      );
      ...
      fc.assert(
        fc.property(
          ...
          (pageKey: keyof PageSEOConfig) => {
            ...
            expect(structuredData.sameAs).toContain('https://github.com/tafara-rugara');
            expect(structuredData.sameAs).toContain('https://linkedin.com/in/tafara-rugara');
          }
        ),
        { numRuns: 10 }
      );
      ...
      fc.assert(
        fc.property(
          ...
          (pageKey: 'projects') => {
            ...
            expect(structuredData.mainEntity.numberOfItems).toBe(4);
            
            // Validate project items
            expect(structuredData.mainEntity.itemListElement).toBeDefined();
            expect(Array.isArray(structuredData.mainEntity.itemListElement)).toBe(true);
            expect(structuredData.mainEntity.itemListElement.length).toBe(4);
      ...
      fc.assert(
        fc.property(
          ...
          (pageKey: 'experience') => {
            ...
            expect(structuredData.hasOccupation.occupationLocation['@type']).toBe('Place');
            ...
            expect(structuredData.hasOccupation.experienceRequirements).toBeDefined();
            expect(structuredData.hasOccupation.experienceRequirements.toLowerCase()).toContain('production');
          }
        ),
        { numRuns: 10 }
      );
  ...
  describe('Image Alt Text and Link Descriptive Text Validation', () => {
        ...
        fc.property(
          ...
          (personalData) => {
            ...
            images.forEach(img => {
              ...
              expect(altText.toLowerCase()).not.toMatch(/^(image|picture|photo)\s/);
            });
          }
        ),
        { numRuns: 10 }
          ...
          (personalData) => {
            ...
            links.forEach(link => {
              ...
              if (ariaLabel) {
                ...
                const hasActionWord = /\b(view|download|visit|connect|contact)\b/i.test(ariaLabel);
                expect(hasActionWord).toBe(true);
                
                // Should be contextually relevant - more flexible matching
                const hasRelevantContext = /\b(cloud|devops|engineer|cv|github|linkedin|projects|portfolio|reference|pdf|featured)\b/i.test(ariaLabel);
    ...
    it('Property 9.12: project links have descriptive text with context', () => {
      fc.assert(
        fc.property(
          fc.constantFrom(...portfolioData.projects),
          (project) => {
            ...
            links.forEach(link => {
              ...
              if (ariaLabel) {
                ...
                if (project.title) {
                  expect(ariaLabel.toLowerCase()).toContain(project.title.toLowerCase().split(' ')[0]);
                }
              } else {
        ...
        fc.property(
          ...
          (config) => {
            ...
            requiredPages.forEach(pageKey => {
              ...
              expect(pageConfig.canonicalUrl).toMatch(/^https:\/\/tafara-rugara\.com/);
        ...
        fc.property(
          ...
          (pageKeys) => {
            ...
            metadataList.forEach(({ metadata, config }) => {
              expect(metadata.creator).toBe('Tafara Rugara');
              expect(metadata.publisher).toBe('Tafara Rugara');
              expect(metadata.openGraph!.siteName).toBe('Tafara Rugara Portfolio');
              expect(metadata.openGraph!.locale).toBe('en_US');
            });
            
            // All pages should have similar keyword themes
            const allKeywords = metadataList.flatMap(({ config }) => 
              config.keywords || []
            );
