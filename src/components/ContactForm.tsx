...
export default function ContactForm({ onSubmit: _onSubmit }: ContactFormProps) {
  ...
  const validate = (data: ContactFormData): FormErrors => {
    ...
    if (!data.message.trim()) e.message = 'Message is required';
      ...
      e.message = 'At least 10 characters required';
    return e;
  };
  ...
  return (
    ...
    >
      ...
      <motion.button
        type="submit"
        disabled={isSubmitting}
        className="relative w-full overflow-hidden rounded-xl px-6 py-4 text-sm font-semibold tracking-wide transition-all duration-200"
        style={
          isSubmitting
            ? {
        ...
        whileHover={
          !isSubmitting
            ? { scale: 1.01, boxShadow: '0 6px 30px rgba(0,212,255,0.50)' }
            : {}
        }
